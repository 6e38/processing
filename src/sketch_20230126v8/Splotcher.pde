...
class Splotcher {
  ...
  boolean draw(PGraphics g) {
    ...
    float Size = 30.0;
    float percent = random(1.0);
    percent = (float)Math.pow(percent, 8) * 0.95 + 0.05;
    float jitterD = Distance * percent;
    float jitterA = random(2.0 * (float)Math.PI);
    float jitterX = (float)Math.cos(jitterA) * jitterD;
    float jitterY = (float)Math.sin(jitterA) * jitterD;
    float w = random(Size) + 5.0;
    float a = random(0.8) + 0.1;
    g.noStroke();
    g.fill(0, 0, 1, a);
